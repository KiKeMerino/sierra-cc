...
def objective_single_basin(trial, basin_data, basin_scalers, exog_cols, exog_cols_scaled):
    ...
    try:
        ...
        # Descomenta este y comenta el de arriba para evaluar el conjunto de validacion en vez de todo el conjunto de datos
...
n_startup_trials = int(n_trials_per_basin * 0.15)
n_warmup_steps = int(n_trials_per_basin*0.25)
...
if study_basin.best_trial is not None:
    ...
    current_basin_metrics = {
        'best_params': best_params_basin,
        '(NSE) full_dataset': best_trial_basin.value,
        # 'train': metrics_train,
        # 'test': metrics_test,
        # 'val': metrics_val
    }
    print(f"Métricas finales para {cuenca_name}:")
    print(f"  Full Dataset (modo prediccion): {best_trial_basin.value:.4f}")
